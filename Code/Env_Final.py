...
class _GymSumo(gym.Env):
    ...
    def __init__(self, args, evalutor):
        ...
        self.high_state = np.array([30,2,180,0.8,0.8,0.8,0.8,1,1.6,20,1,1])
        self.low_state = np.array([-1,-2,-180,-0.8,-0.8,-0.8,-0.8,0,-1.6,-20,-1,-1])
    ...
    def step(self, action):
        ...
        self.states, state = self.stepPhysics(self.vehcura)
    ...
    def stepPhysics(self, action):
        ...
        edge_x = edge[0]
        edge_y = edge[1]
        x_ego = Feature_ego([action[0],action[1],action[2],action[3]],action[4],self.start_point,_GymSumo.Dys)
        # x_ego = Feature_ego([50,50,50,50],action[4],self.start_point,_GymSumo.Dys)
        time.sleep(0.01)
        traci.simulationStep(self.steps/100)
        plt.figure(1)
        plt.clf()
        plt.text(0.5, 0.5, "Hello SUMO!", fontsize=12, ha='center', va='center')
        plt.axis('off')
        ...
        if Vx < 5 and action[0] > 0:
            Slip_FL = 0.1
            Slip_FR = 0.1
            Slip_RL = 0.1
            Slip_RR = 0.1
        elif Vx < 5 and action[0] < 0:
            Slip_FL = -0.1
            Slip_FR = -0.1
            Slip_RL = -0.1
            Slip_RR = -0.1
        ...
        if front_veh and front_veh_dis < 20 and junction == 1:
            ...
            if Vx_del < 0:
                Vx_del = 0
            if ego_lane[0] == 'D':
                max_Vx = traci.vehicle.getSpeed(self.neiberId[0][0]) * math.cos(math.radians(traci.vehicle.getAngle(self.neiberId[0][0]) + 90)) + Vx_del - 0.5
            else:
        ...
        if not(ego_lane == 'E3_6' or ego_lane == 'D2_2'):
            Vx_max_ = self.random_flow.NeiChangeLaneVx(self.neiberId,delta_x)
            if Vx_max_ < 24:
                max_Vx = Vx_max_
        end_time = time.time()
        e_time = start_time - end_time

        return (X,Y,Vx,Vy,Yaw,delta_x,delta_y,dis2des_x,dis2des_y,Slip_FL,Slip_FR,Slip_RL,Slip_RR,self.lane_keep,ego_Lateral_pos,Vx - max_Vx,del_ego_Lateral_pos,junction), (Vx,Vy,Yaw,Slip_FL,Slip_FR,Slip_RL,Slip_RR,self.lane_keep,ego_Lateral_pos,Vx - max_Vx,del_ego_Lateral_pos,junction) 


    def reset(self):
        start_X = self.start_point[0]
        start_Y = self.start_point[1]
        _GymSumo.Dys = Dynamic_step()
        self.veh_nei = self.veh_nei_real
        result=_GymSumo.Dys.step(start=0,end=0,step=0.001)
        ...
        self.states = [ego_X,ego_Y,ego_Vx,ego_Vy,ego_Angle,delta_x,delta_y,dis2des_x,dis2des_y,Slip_FL,Slip_FR,Slip_RL,Slip_RR,self.lane_keep,self.ego_Lateral_pos,ego_Vx - max_Vx,0,1]
        
        state = [ego_Vx,ego_Vy,ego_Angle,Slip_FL,Slip_FR,Slip_RL,Slip_RR,self.lane_keep,self.ego_Lateral_pos,ego_Vx - max_Vx,0,1]
        self.delta_f = 0
        self.Tt = 0
        # state_code = (2 * (state - self.low_state) / (self.high_state - self.low_state) - 1)
        state_code = state
    ...
    def Reward(self, action, del_delta_f, del_Tt):
        ...
        if ((Tt > 0 and self.states[15] < -0.5)):
            Tt_reward = 0.5
        elif ((Tt < 0 and self.states[15] > 0.5)):
            Tt_reward = abs(Tt / 5000)    
        else:
            Tt_reward = 0
        ...
        reward = Vx_reward + delta_f_reward * factor_delta_f + KeepLane_reward + Tt_reward + correct_KeepLane_reward + R_slip 
...
def env_creator(**kwargs):
    return TimeLimit(_GymSumo(**kwargs), 10000)
